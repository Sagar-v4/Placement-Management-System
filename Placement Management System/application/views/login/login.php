          ...
          <div class="col-xl-4 col-lg-5 col-md-8 col-sm-10 d-flex flex-column mx-auto">
            ...
            <?php
              if ( !empty($this->session->flashdata('msg')) ) {
                ...
                <span class='alert-text'><strong class='text-white text-bold'>".$this->session->flashdata('msg')."</strong></>
                <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'>
                </button></div>";
              }

              if ( !empty($this->session->flashdata('signuplogMsg')) ) {
                echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
                <span class='alert-text'><strong class='text-white text-bold'>".$this->session->flashdata('signuplogMsg')."</strong></span>
                <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'>
                </button></div>";
              }           
            ?>
              ...
              <div class="card-body">
                ...
                <form action="<?= base_url().'login/login/authenticate'; ?>" name="loginForm" id="loginForm" method="POST" role="form text-left" autocomplete="off">
                  
                  <label for="email" class="form-control-label">Email</label>
                  <div class="mb-3">
                    <input type="email" value="<?= set_value('email');?>" name="email" id="email" class="form-control <?= form_error('email') ? 'is-invalid' : '';?>" placeholder="Email" aria-label="Email" aria-describedby="email-addon" />
                  </div>
                  
                  <label for="password" class="form-control-label">Password</label>
                  <div class="mb-3">
                    <input type="password" value="<?= set_value('password');?>" name="password" id="password" class="form-control <?= form_error('password') ? 'is-invalid' : '';?>" placeholder="Password" aria-label="Password" aria-describedby="password-addon" />
                  </div>
                  
                  <?php
                   if ( get_cookie('email') and get_cookie('password') ) {
                  ...
                  <a href="<?= base_url().'forgot/confirm_email'; ?>" class="link text-secondary text-sm" >Forgot Password ?</a>
              ...
              <div class="card-footer text-center pt-0 px-lg-2 px-1">
                <p class="mb-4 text-sm mx-auto">
                  ...
                  <a href="<?= base_url().'registration/register/index'; ?>" class="text-info text-gradient font-weight-bold">Create One.</a>
                </p>
              </div>
            </div>
          </div>
