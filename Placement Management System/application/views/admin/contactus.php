...
<div class="wrapper">
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <?php if ( $userInfo['power'] == 1 ) { ?>
            <li class="nav-item">
              <a href="<?= base_url().'admin/contactus/index';?>" class="nav-link active">
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="card-tools">
                  <div class="input-group input-group-sm" style="width: 150px;">
                    <input type="text" name="table_search" id="table_search_inbox" class="form-control float-right" placeholder="Search">
              ...
              <div class="card-body table-responsive p-0" style="min-height: 310px; max-height: 538px;">
                <table class="table table-head-fixed text-nowrap table-hover" id="inboxTable">
                  <thead>
                    <tr>
                      <th onclick="w3.sortHTML('#inboxTable', '.item', 'td:nth-child(1)')" style="cursor:pointer">#</th>
                      <th onclick="w3.sortHTML('#inboxTable', '.item', 'td:nth-child(2)')" style="cursor:pointer">Email</th>
                      <th onclick="w3.sortHTML('#inboxTable', '.item', 'td:nth-child(3)')" style="cursor:pointer">Subject</th>
                      <th onclick="w3.sortHTML('#inboxTable', '.item', 'td:nth-child(4)')" style="cursor:pointer">Message</th>
                      <th onclick="w3.sortHTML('#inboxTable', '.item', 'td:nth-child(5)')" style="cursor:pointer">Time</th>
                    </tr>
                  </thead>

                  <tbody id="inboxes">
                      
                      <?php if ( !empty($inboxes) ) { $i = 0; rsort($inboxes); foreach ($inboxes as $inbox) { $i++;?>
                        <tr class="item" >
                          <td><?= $i ;?></td>
                          <td><?= $inbox['contact_email'] ;?></td>
                          <td><?= $inbox['contact_subject'] ;?></td>
                          <td><?= $inbox['contact_message'] ;?></td>
                          <td><?= date('j-n-Y h:i:s A', strtotime($inbox['contact_created_at'])) ;?></td>
                        </tr>
                      <?php } } else { ?>
                        <tr>
  ...
  $(document).ready(function(){
    $("#table_search_inbox").on("keyup", function() {
      var value = $(this).val().toLowerCase();
      $("#inboxes tr").filter(function() {
