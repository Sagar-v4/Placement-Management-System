...
class Applied extends CI_Controller {
	...
	public function index()	{
		...
		$this->db->where('student_id', $userInfo['sid']);
		$reqLinks = $this->db->get('pms_interview_pass')->result_array();
		...
		if ( !empty($requirements) ) { 
			...
			foreach ($requirements as $requirement) { 
				...
				$requirements[$j]['link'] = NULL;

				foreach($reqLinks as $reqLink) {

					if ( $requirement['company_requirement_id'] == $reqLink['company_requirement_id'] ) {
						
						$requirements[$j]['link'] = $reqLink['interview_link'];
					}
				}
				$j++;
			}
		...
		$data['reqStatuses'] = $reqStatuses;
		$data['quickNotifications'] = $quickNotifications;

		$this->load->view('student/applied', $data);
	...
	public function apply( $comId, $comReqId, $studId ) {
		...
		if ( empty($reqStatus) ) {
			...
			redirect(base_url().'student/applied/index');
			...
			if ( $reqStatus['student_applied_req_status'] == 1 ) {
				...
				$this->db->where('student_id', $studId );
				$this->db->update( 'pms_student_applied_req', $update );
				$this->session->set_flashdata('updateRSMsg', 'Your Applied again Successfully.');
				redirect(base_url().'student/applied/index');
